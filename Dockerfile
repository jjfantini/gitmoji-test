...
WORKDIR /workspaces/gitmoji-test/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/gitmoji-test/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/gitmoji_test/ && touch src/gitmoji_test/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/gitmoji-test/
